...
- **输入框有文字**发送 = **插入到内部轮次**：忙时把消息插进 agent 当前内部轮次的间隙
  （push 进流 + `interrupt()`，保留上下文接着干，像真实 Claude）；空闲时正常开一轮。
- **输入框为空**发送（手机点发送 / 终端空行回车）= **硬打断**：掐断当前这轮、丢弃在途工作。
- 手机发送按钮文案兼当忙闲指示：**空闲显示「发送」**（这轮说完了）、在跑时显示「插入」或「打断」。
- 手机往前滑到顶自动加载更早的历史消息。

> ⚠️ 这套发消息语义（尤其「插入到内部轮次」）是反复讨论后定的核心行为，改动前务必看
