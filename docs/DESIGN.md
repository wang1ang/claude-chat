...
> **统一术语**：**排队** = 忙时发消息，排进 agent 内部轮次（保留上下文接着处理，像真实 Claude）；
> **打断** = 空发送，掐断当前生成、丢弃在途工作（Ctrl-C 语义）。全项目一致用这两个词。

- **输入框有文字**发送 = **排队**：忙时把消息排进 agent 当前内部轮次
  （push 进流 + SDK `interrupt()`，保留上下文接着处理，像真实 Claude）；空闲时正常开一轮。
- **输入框为空**发送（手机点发送 / 终端空行回车）= **打断**：掐断当前这轮、丢弃在途工作。
- 手机发送按钮文案兼当忙闲指示：**空闲显示「发送」**（这轮说完了）、在跑时显示「排队」或「打断」。
- 手机往前滑到顶自动加载更早的历史消息。

> ⚠️ 这套发消息语义（尤其「排队到内部轮次」）是反复讨论后定的核心行为，改动前务必看
> [`REQUIREMENTS.md`](./REQUIREMENTS.md)——那里记录了所有已确定的需求与踩过的坑，别改回旧模型。
